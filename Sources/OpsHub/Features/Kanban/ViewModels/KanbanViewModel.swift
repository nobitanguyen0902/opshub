...
@MainActor final class KanbanViewModel: ObservableObject {
    ...
    func refresh() async { guard !refreshing else { return }; refreshing = true; defer { refreshing = false }; do { let value = try await Task.detached { try self.reader.loadBoard() }.value; snapshot = value; errorMessage = nil } catch { errorMessage = error.localizedDescription } }
